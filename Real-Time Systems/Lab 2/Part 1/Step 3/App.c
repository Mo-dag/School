  ...
  void Tone_generator(App *self, int unused){   // Tiny timber kräver denna form !
    ...
    if(deadline == true){
      SEND(USEC(500),USEC(100),self,Tone_generator,0);}  // 500 >> 1 kHz för att : 1 kHz =1000, 500µs för 0 + 500µs för 1 = en period 
    else{
...
void print_background_loop_range(App *self){
  char msg[50];
  sprintf(msg, "Back ground loop range is : %i\n", self->background_loop_range);
  SCI_WRITE(&sci0, msg);
}

void print_deadline_enable(App *self){
  char msg[50];
  if(deadline){
    sprintf(msg, "Dead Line is Enabled\n");
  }
  else{sprintf(msg, "Dead Line is Disabled\n");}
  SCI_WRITE(&sci0, msg);
}
  ...
  void E(App *self){
    ...
    if(self->buffer[0]== '-'){
      ...
      if(self->background_loop_range <= 20000){
        self->background_loop_range +=500;
        print_background_loop_range(self);
      }
    }
